...
while (rs.next()) {
    ...
    public boolean addPerson(Person person, String altText) {
    ...
    String sqlPersonImage = "INSERT INTO PersonImages (PersonID, image_url, alt_text) VALUES (?, ?, ?)";
    ...
    try {
        ...
        stmtPerson.setString(3, person.getDateOfBirth());  // Giả sử đúng định dạng
        ...
        stmtAddress.setInt(1, personID);
        stmtAddress.setString(2, person.getAddress());
        stmtAddress.setBoolean(3, true);  // Giả sử là địa chỉ chính
        ...
        stmtPhone.setInt(1, personID);
        stmtPhone.setString(2, person.getPhone());
        stmtPhone.setBoolean(3, true);  // Giả sử là số điện thoại chính
        ...
        // 4. Chèn vào bảng PersonImages
        PreparedStatement stmtImage = connection.prepareStatement(sqlPersonImage);
        stmtImage.setInt(1, personID);
        stmtImage.setString(2, person.getImage());
        stmtImage.setString(3, altText);
int rowsAffectedImage = stmtImage.executeUpdate();
        if (rowsAffectedImage == 0) {
            throw new SQLException("Failed to insert into PersonImages table");
        }
